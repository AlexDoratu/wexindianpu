...
如果你看到 `在项目根目录未找到 app.json`：

1. 请直接导入仓库根目录 `wexindianpu/`（不要只导入 `server/`）
2. 本项目已新增 `project.config.json`，并设置 `miniprogramRoot: "miniprogram/"`
3. 若仍报错，在微信开发者工具中检查：
   - 详情 -> 本地设置 -> 小程序代码目录 是否为 `miniprogram/`
   - 基础库版本可使用 `3.14.3` 或稳定版
